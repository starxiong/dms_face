...
int main() {
  ...
  if (!std::filesystem::exists(path + "_result")) {
    fs::create_directories(path + "_result");
  }
  ...
  for (int i = 0; i < files.size(); i++) {
    ...
    if (pos != std::string::npos) {
      ...
      } else if (std::filesystem::exists(video_path.substr(0, pos) + ".avi")) {
    ...
    while (true) {
      ...
      if ((t_diff > 0.5 || angle_deg > 10.0) && frame_id > 1) {
        ofs << filename << ", number: " << number << ", frame_id: " << frame_id
            << ", angle_deg: " << angle_deg << ", t_diff: " << t_diff
            << std::endl;
      }
      cv::imwrite(path + "_result/" + std::to_string(frame_id) + ".jpg", frame);
      R = next_R, t = next_t;
